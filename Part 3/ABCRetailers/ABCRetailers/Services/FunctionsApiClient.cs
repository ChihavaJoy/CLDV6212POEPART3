...
namespace ABCRetailers.Services
{
    public class FunctionsApiClient : IFunctionsApi
    {
        ...
        public async Task<Order> CreateOrderAsync(string customerId, string productId, int quantity, DateTime orderDate)
        {
            // Prepare payload including orderDate
            var payload = new
            {
                customerId,
                productId,
                quantity,
                orderDateUtc = orderDate.ToUniversalTime() // send in UTC to match your DTO
            };
