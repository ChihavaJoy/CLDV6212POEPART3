...
namespace ABCRetailers.Functions.Functions
{
    public class OrdersFunctions
    {
            ...
            [HttpTrigger(AuthorizationLevel.Anonymous, "post", Route = "orders")] HttpRequestData req)
        {
            ...
            // --- Small logging tweak to confirm the order was saved ---
            try
            {
                var savedOrder = await orders.GetEntityAsync<OrderEntity>("Order", order.RowKey);
                Console.WriteLine($"Order saved! Id: {savedOrder.Value.RowKey}, Username: {savedOrder.Value.Username}");
            }
            catch (Exception ex)
            {
                Console.WriteLine($"Failed to fetch saved order: {ex.Message}");
            }
